      ...
      INTEGER(4), parameter :: NX=11    !ITO CHANGE !x-direction
      INTEGER(4), parameter :: NY=44    !ITO CHANGE  !y-direction
      INTEGER(4), parameter :: NZ=10    !ITO CHANGE !z-direction
...
!     ,OMEX(NX+4,NY+4,NZ+4),OMEY(NX+4,NY+4,NZ+4)&   !GAL?øΩ?øΩ?øΩ?øΩOMEX,Y,Z?øΩ?øΩOMEXG,YG,ZG?øΩ…ïœùX
!     ,OMEZ(NX+4,NY+4,NZ+4),HKW(NX+4,NY+4,NZ+4)&
!     ,HKU(NX+4,NY+4,NZ+4),HKV(NX+4,NY+4,NZ+4)&
!     ,DD1(NX+4,NY+4,NZ+4),DD2(NX+4,NY+4,NZ+4)&   !GAL?øΩ?øΩ?øΩ?øΩDD1,2,3?øΩ?øΩDD1G,2G,3G?øΩ…ïœùX
!     ,DD3(NX+4,NY+4,NZ+4)&
...
!
     ...
     ,USRT(:,:,:),VSRT(:,:,:)&
     ,WSRT(:,:,:),WST(:,:,:)&
     ,OMEXG(:,:,:),OMEYG(:,:,:)&   !OMEX?øΩ?øΩOMEXG,OMEY?øΩ?øΩOMEYG
     ,OMEZG(:,:,:),HKW(:,:,:)&     !OMEZ?øΩ?øΩOMEZG
     ,HKU(:,:,:),HKV(:,:,:)&
     ,DD1G(:,:,:),DD2G(:,:,:)&     !DD1?øΩ?øΩDD1G,DD2?øΩ?øΩDD2G
     ,DD3G(:,:,:)&                 !DD3?øΩ?øΩDD3G
     ,AU(:,:,:),W0(:,:,:)&
     ,AP1(:,:,:),AP2(:,:,:)&
     ,BTE(:,:,:),AP3(:,:,:)
...
!C
      REAL(8) :: UA(NX+5),C3
      REAL(8) :: UKG(NX+4,NY+4,NZ+4),UKE(NX+4,NY+4,NZ+4)    !SAL?øΩ≈ÇÔøΩ?øΩ?øΩ?øΩl?øΩÃéÔøΩ
      REAL(8) :: VKG(NX+4,NY+4,NZ+4),VKE(NX+4,NY+4,NZ+4)    !SAL?øΩ≈ÇÔøΩ?øΩ?øΩ?øΩl?øΩÃéÔøΩ
      REAL(8) :: MYTGENE(NX+4,NY+4,NZ+4),MYTEENE(NX+4,NY+4,NZ+4)    !SAL?øΩ≈ÇÔøΩ?øΩ?øΩ?øΩl?øΩÃéÔøΩ
      REAL(8) :: AK0(NX+4,NY+4,NZ+4)
      REAL(8) :: UK(NX+4,NY+4,NZ+4),VK(NX+4,NY+4,NZ+4),CKC    !SAL?øΩ≈ÇÔøΩ?øΩ?øΩ?øΩl?øΩÃéÔøΩ
!
!C----------Original Dimension----------C
!      REAL(8) ::  HKG(NX+4,NY+4,NZ+4),HKE(NX+4,NY+4,NZ+4)& !HKG?øΩ?øΩHKGENE?øΩCHKE?øΩ?øΩHKEENE
!     ,HKGG(NX+4,NY+4,NZ+4),HKEE(NX+4,NY+4,NZ+4)&
!     ,MYTG(NX+4,NY+4,NZ+4),MYTE(NX+4,NY+4,NZ+4)&   !MYTG?øΩ?øΩMYTGENE?øΩCMYTE?øΩ?øΩMYTEENE
...
!     ,HKD(NX+4,NY+4,NZ+4),HKDD(NX+4,NY+4,NZ+4)&            !HKD?øΩ?øΩHKDENE
!     ,MYTD(NX+4,NY+4,NZ+4),WCK(NX+4,NY+4,NZ+4)&            !MYTD?øΩ?øΩMYTDENE
...
!C*****************************************************************
    ...
    fx(-1:IMAX+4,-1:JMAX+4,-1:KMAX+4,-1:3,-1:3,-1:3) &
     ...
     ,OMEXG(-1:IMAX+4,-1:JMAX+4,-1:KMAX+4),OMEYG(-1:IMAX+4,-1:JMAX+4,-1:KMAX+4) &   !OMEX?øΩ?øΩOMEXG,OMEY?øΩ?øΩOMEYG
     ,OMEZG(-1:IMAX+4,-1:JMAX+4,-1:KMAX+4),HKW(-1:IMAX+4,-1:JMAX+4,-1:KMAX+4) &     !OMEZ?øΩ?øΩOMEZG
     ,HKU(-1:IMAX+4,-1:JMAX+4,-1:KMAX+4),HKV(-1:IMAX+4,-1:JMAX+4,-1:KMAX+4) &
     ,DD1G(-1:IMAX+4,-1:JMAX+4,-1:KMAX+4),DD2G(-1:IMAX+4,-1:JMAX+4,-1:KMAX+4) &     !DD1?øΩ?øΩDD1G,DD2?øΩ?øΩDD2G
     ,DD3G(-1:IMAX+4,-1:JMAX+4,-1:KMAX+4) &                 !DD3?øΩ?øΩDD3G
...
!C//////////////  Gravity(AG) & Pressure Gradient(AP) ////////////
        ...
        AG3=0.0
        DT1=DT*1.0
        B00=RC*(1.0+CM0)/(RS+RC*CM0)    !B0?øΩ?øΩB00, SUMO23/05/04
...
!
            ...
            DO 600 J=3,JMAX+1
!
                RE1G(I,J,K)=SQRT(ABS(US(I,J,K)-U(I,J,K))**2 &   !RE1?øΩ?øΩRE1G, SUMO23/05/04
...
!                          *(0.650-CM(I,J,K))**(-2.70)
!                          *(1.0-CM(I,J,K))**(-2.70)
!                          *(1.0+0.150*RE1G(I,J,K)**0.6870)   !RE1?øΩ?øΩRE1G, SUMO23/05/04
...
!
            ...
            DO 700 J=3,JMAX+1
!
                OMEXG(I,J,K)=RC/(RS+RC*CM0)*CL0* &  !OMEX?øΩ?øΩOMEXG, SUMO23/05/04
                            ((W(I,J+1,K)-W(I,J-1,K))*0.50*EY(J) &
                            -(V(I,J,K+1)-V(I,J,K-1))*0.50*DZ(K))
                OMEYG(I,J,K)=RC/(RS+RC*CM0)*CL0* &  !OMEY?øΩ?øΩOMEYG, SUMO23/05/04
                            ((U(I,J,K+1)-U(I,J,K-1))*0.50*DZ(K) &
                            -(W(I+1,J,K)-W(I-1,J,K))*0.50*GX(I))
                OMEZG(I,J,K)=RC/(RS+RC*CM0)*CL0* &  !OMEZ?øΩ?øΩOMEZG, SUMO23/05/04
...
!C
                TAUP(I,J,K)=DR**2/(18.0*RED)*RS/RC &
                            /(1.0-CM(I,J,K))**(-2.70) &
!C     &            /(1.0+0.150*RE1G(I,J,K))**0.6870   !RE1?øΩ?øΩRE1G, SUMO23/05/04
!                            *(1.0+0.150*RE1G(I,J,K)**0.6870)   !RE1?øΩ?øΩRE1G, SUMO23/05/04
                            *(1.0+0.4320*RE1G(I,J,K)**0.6870)   !SUMO23/10/23
 750  CONTINUE
!
...
!                        *(1-CM(I,J,K))**(-2.70)*(1.0+0.150*RE1G(I,J,K)**0.687)   !RE1?øΩ?øΩRE1G, SUMO23/05/04
...
!
            IF (CM(I,J,K).GT.1.0E-7) THEN
                UUSR(I,J,K)=HKU(I,J,K)*(AU(I,J,K)**2+B00*W0(I,J,K)**2) &    !B0?øΩ?øΩB00, SUMO23/05/04
                                      /(AU(I,J,K)**2+W0(I,J,K)**2)
                VVSR(I,J,K)=HKV(I,J,K)*(AU(I,J,K)**2+B00*W0(I,J,K)**2) &    !B0?øΩ?øΩB00, SUMO23/05/04
                                      /(AU(I,J,K)**2+W0(I,J,K)**2)
                WWSR(I,J,K)=HKW(I,J,K)*(AU(I,J,K)**2+B00*W0(I,J,K)**2) &    !B0?øΩ?øΩB00, SUMO23/05/04
                                      /(AU(I,J,K)**2+W0(I,J,K)**2)
            ELSE
                UUSR(I,J,K)=0.0
                VVSR(I,J,K)=0.0
...
!********** OKUYUKI & TEMAE NO JYOUKEN **********!
            ...
            !    !NO-SLIP
            !    !WALL ÇÃà íuÇ…ã≠êßìIÇ…0Çó^Ç¶ÇÈ
...
!C         SK(I,J,K)=0.0
!C         HK(I,J,K)=0.0
!C         MYT(I,J,K)=(CSS*DET(I,J,K))**2.0
!C     &         *(2.0*( (2.0*UKG(I,J,K))**2+(2.0*VKE(I,J,K))**2
!C     &                +(2.0*WKD(I,J,K))**2+(UKE(I,J,K)+VKG(I,J,K))**2
!C     &                +(UKD(I,J,K)+WKG(I,J,K))**2
!C     &                +(VKD(I,J,K)+WKE(I,J,K))**2 ))**0.50
...
!c      &                   IGL(1,5)+1,IETA(1,5)
        ...
        !I=1
        !DO 3224 K=1,KMAX+4
        !DO 3224 J=IGL(I,K)+1,IETA(I,K) !ïœìÆù¨ï™ÇëŒùîï™ïzÇ…ïtâ¡
...
!C=================
        IMAX=11.0   !ITO CHANGE
        JMAX=44.0   !ITO CHANGE
        KMAX=10.0   !ITO CHANGE
!C=================
...
!
            OPEN(10,FILE= 'INPUT/10_HS.txt')
                READ(10,*) HSTIME   !HS?øΩJ?øΩn?øΩ?øΩ?øΩ‘ÇÔøΩ?øΩ?øΩ?øΩ(s)
            CLOSE(10)
!            NMIN=NINT(HSTIME)
