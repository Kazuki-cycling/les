...
!     ,OMEX(NX+4,NY+4,NZ+4),OMEY(NX+4,NY+4,NZ+4)&   !GAL????????????????????????????OMEX,Y,Z??????????????OMEXG,YG,ZG?????????X
!     ,OMEZ(NX+4,NY+4,NZ+4),HKW(NX+4,NY+4,NZ+4)&
!     ,HKU(NX+4,NY+4,NZ+4),HKV(NX+4,NY+4,NZ+4)&
!     ,DD1(NX+4,NY+4,NZ+4),DD2(NX+4,NY+4,NZ+4)&   !GAL????????????????????????????DD1,2,3??????????????DD1G,2G,3G?????????X
!     ,DD3(NX+4,NY+4,NZ+4)&
...
!C
      REAL(8) :: UA(NX+5),C3
      REAL(8) :: UKG(NX+4,NY+4,NZ+4),UKE(NX+4,NY+4,NZ+4)    !SAL??????????????????????????????l?????????
      REAL(8) :: VKG(NX+4,NY+4,NZ+4),VKE(NX+4,NY+4,NZ+4)    !SAL??????????????????????????????l?????????
      REAL(8) :: MYTGENE(NX+4,NY+4,NZ+4),MYTEENE(NX+4,NY+4,NZ+4)    !SAL??????????????????????????????l?????????
      REAL(8) :: AK0(NX+4,NY+4,NZ+4)
      REAL(8) :: UK(NX+4,NY+4,NZ+4),VK(NX+4,NY+4,NZ+4),CKC    !SAL??????????????????????????????l?????????
!
!C----------Original Dimension----------C
...
!*****************************************************************
!
!MAINROUTINEが計算の実行場所　CALLでMAINROUTINE(関数)を呼び出す
!
...
!   2022/10/29, Sat., [.f90 added QOpenMP] completed without Subroutine GAL by Takashi Inoue.
    ...
    USE MF_SF !use variable
    USE OMP_LIB !use openMP並列計算
    IMPLICIT NONE 
...
!
!!!!!                         CALL READ_HS !ホットスタートの場合にはここをオンする（ホットスタート：途中から計算を行う）
!
! ここが時間発展の計算，計算時間の計測                        
            DO 100 N=N1,NMAX
!!!!!            DO 100 N=NMIN,NMAX
...
!C{89 line}             *******************
                        IT7=MCLOCK()
                         CALL NSE !ナビエストークス方程式を解いて，移流・粘性項を算出？
                        IT8=MCLOCK()
...
!C
                        IT11=MCLOCK()
                         CALL SMAC !SMAC（圧力ポアソン方程式）を解いて，速度を発散ゼロに補正？
                        IT12=MCLOCK()
...
!$OMP DO
        ...
        DO 50 J=1,JMAX+3
            ...
            !
            MYT(I,J,K)=CS*ABS(HK(I,J,K))**0.50*DET(I,J,K) !ito260316
...
!********** OKUYUKI & TEMAE NO JYOUKEN **********!
            ...
            !    !NO-SLIP
            !    !WALL ?????u??????????????I??????0??????^????????????
            !    HKDENE(I,J,3)=(HK(I,J,4)-0.0)*DZ(3)/1.500
            !    HKDD(I,J,3)=(HK(I,J,4)+0.0)*DZ(3)**2
            !    MYTDENE(I,J,3)=(MYT(I,J,4)-0.0)*DZ(3)/1.500
            !
            !    HKDENE(I,J,KMAX+2)=(0.0-HK(I,J,KMAX+1))*DZ(3)/1.500
            !    HKDD(I,J,KMAX+2)=(0.0+HK(I,J,KMAX+1))*DZ(3)**2
            !    MYTDENE(I,J,KMAX+2)=(0.0-MYT(I,J,KMAX+1))*DZ(3)/1.500
...
!
            !    !NO-SLIP
            !    !WALL　底面の境界条件  !ito260226
            !    HKEENE(I,3,K)=(HK(I,4,K)-0.0)*EY(3)/1.500
            !    HKEE(I,3,K)=(HK(I,4,K)+0.0)*EY(3)**2
            !    MYTEENE(I,3,K)=(MYT(I,4,K)-0.0)*EY(3)/1.500
            !    !!surface　水面の境界条件  !ito260226
            !    !HKEENE(I,IETA(I,K),K)=(0.0-HK(I,IETA(I,K)-1,K))/(0.500/EY(3)+DETA(I,K))
            !    !HKEE(I,IETA(I,K),K)=(0.0-(DETA(I,K)*EY(3)-0.5)*HK(I,IETA(I,K),K)-HK(I,IETA(I,K),K)-HK(I,IETA(I,K)-1,K))*EY(3)**2
            !    !MYTEENE(I,IETA(I,K),K)=(0.0-MYT(I,IETA(I,K)-1,K))/(0.500/EY(3)+DETA(I,K))
...
!C     &                +AK0(I,J,K))
            ...
            !    DK(I,J,K)=DT*(3.000*MYT(I,J,K)*DZ(K)**2+ &
            !                  2.000*MYT(I,J,K)*(EY(J)**2+GX(I)**2) &
            !                  +AK0(I,J,K)+2.0*TEI1(I,J,K))
            !END IF
            !IF(J.EQ.3) THEN
            !    DK(I,J,K)=DT*(3.000*MYT(I,J,K)*EY(J)**2+ &
            !                  2.000*MYT(I,J,K)*(DZ(K)**2+GX(I)**2) &
            !                  +AK0(I,J,K)+2.0*TEI1(I,J,K))
            !END IF
            
            !エネルギー方程式の誤差量
            !IF(J.EQ.IETA(I,K)) THEN
            !    ! surface DK !ito260226
            !    DK(I,IETA(I,K),K)=DT*((DETA(I,K)*EY(3)+0.500)*MYT(I,J,K)*EY(J)**2+ &
            !                  2.000*MYT(I,J,K)*(DZ(K)**2+GX(I)**2) &
            !                  +AK0(I,J,K)+2.0*TEI1(I,J,K))
            !END IF
...
!$OMP DO
        ...
        DO 1781 K=Kpier_R,Kpier_L
            ...
            IF(I.EQ.Ipier_U-1) THEN               
                !WALL ?????u??????????????I??????0??????^????????????
                DK(I,J,K)=DT*(3.000*MYT(I,J,K)*GX(I)**2+ &
...
!C     Smagorinsky Model !2.00????0.5???X

!        SK(I,J,K)=0.0
!        HK(I,J,K)=0.0
!        MYT(I,J,K)=(0.165*DET(I,J,K))**2.0 &
!             *(0.500*( (2.0*UKG(I,J,K))**2+(2.0*VKE(I,J,K))**2 &
...
!c      &                   IGL(1,5)+1,IETA(1,5)
        ...
        !I=1
        !DO 3224 K=1,KMAX+4
        !DO 3224 J=IGL(I,K)+1,IETA(I,K) !???????????????????????Ν??????????z????t??????
...
!C=================
        ...
        IF(DEPMODE.EQ.1.0) SLOPE=0.001482d0   !ITO CHANGE   !SUMO23/03/23
        !SLOPE=0.0
...
!C**********  MITUDO & other cofe **********C
        ...
        GG=980.00
        DR=0.13350
        Qin=0.000702D0   !ITO CHANGE  !m^3/s
        CSmax=0.0
        !CSmax=1.00  !0.600
...
!********** DEPTH AND ETA SETTING **********!
        ...
        DO 917 J=1,JMAX+3
            ...
            DEP(I,K)=(((QQQ(I)/(FLOAT(KMAX)/DZ(3)))**2)*(RNM(I,K)**2)/SLOPE)**(3.0/10.0)  !初期水深を等流水深で与える　!SUMO241113
 ...
 917    CONTINUE
        ...
        DEPC=(((QQQ(3)/(FLOAT(KMAX)/DZ(3)))**2)/GG)**(1.0/3.0) !限界水深
...
!********** VELOCITY SETTING **********!
        ...
        DO 912 K=1,KMAX+4
            ...
            !USTAB(I,K)=CFF(I,K)**0.500*UT(I,K)
            USTAB(I,K)=1.3788341D0 !ITO CHANGE
